...
class PointageBot(BaseBot):
    ...
    def __init__(self):
        ...
        self.badge_mapping: Dict[str, int] = {}  # badge ZK → employee_id Odoo
        self.name_mapping: Dict[str, int] = {}  # name ZK → employee_id Odoo
    ...
    def initialize(self) -> bool:
        ...
        # Charge ou construit le mapping employés
        ...
        if self.mapping_file.exists():
            try:
                with open(self.mapping_file, 'r') as f:
                    ...
                    # Vérifie si le mapping n'est pas trop vieux (> 1 jour)
    ...
    def _build_mapping(self):
        ...
        # Construit aussi un mapping par nom (fallback)
        self.name_mapping = {
    ...
    def collect(self, start_date: datetime = None, end_date: datetime = None, **kwargs) -> List[Pointage]:
        ...
        Args:
            start_date: Date de début (défaut: dernier sync)
    ...
    def analyze(self, pointages: List[Pointage], **kwargs) -> List[SyncResult]:
        ...
        Args:
            pointages: Liste de pointages à synchroniser
        ...
        # Groupe les pointages par employé pour traitement ordonné
        ...
        for emp_id, emp_pointages in pointages_by_employee.items():
            # Trouve l'employé Odoo correspondant
            ...
            # Traite les pointages de cet employé (triés par timestamp)
            sorted_pointages = sorted(emp_pointages, key=lambda p: p.timestamp)

            # Pour les pointages AUTO, on doit déterminer IN/OUT
            if any(p.punch_type == 'AUTO' for p in sorted_pointages):
                sorted_pointages = self._resolve_auto_punches(sorted_pointages, odoo_emp_id)
    ...
    def _resolve_auto_punches(self, pointages: List[Pointage], odoo_emp_id: int) -> List[Pointage]:
        """
        Résout les pointages en mode AUTO (punch_state=255).
        Alterne entre IN et OUT selon l'état de présence Odoo.

        Args:
            pointages: Liste de pointages triés par timestamp
            odoo_emp_id: ID employé Odoo

        Returns:
            Liste de pointages avec punch_type résolu (IN/OUT)
        """
        resolved = []

        # Vérifie s'il y a une présence ouverte dans Odoo
        open_attendance = self.odoo_client.get_open_attendance(odoo_emp_id)
        expect_out = open_attendance is not None  # Si présence ouverte, on attend un OUT

        current_date = None

        for p in pointages:
            # Reset au début de chaque jour (premier pointage = IN)
            if current_date != p.timestamp.date():
                current_date = p.timestamp.date()
                # Vérifie si déjà une présence ouverte ce jour-là
                open_attendance = self.odoo_client.get_open_attendance(odoo_emp_id)
                expect_out = open_attendance is not None

            if p.punch_type == 'AUTO':
                # Détermine IN ou OUT selon l'alternance
                resolved_type = 'OUT' if expect_out else 'IN'
                p = Pointage(
                ...
                expect_out = not expect_out  # Alterne pour le prochain
    ...
    def _process_pointage(self, pointage: Pointage, odoo_emp_id: int) -> SyncResult:
        ...
        try:
            ...
            # Vérifie si ce pointage existe déjà
            ...
            if pointage.punch_type == 'IN':
                # Check-in: vérifie d'abord s'il y a une présence ouverte
                open_attendance = self.odoo_client.get_open_attendance(odoo_emp_id)

                if open_attendance:
                    # Il y a déjà une présence ouverte - on la ferme d'abord
                    # Parse le check_in de la présence ouverte
                    ...
                    if pointage.timestamp > open_checkin:
                        # Le nouveau pointage est APRES le check-in ouvert - on peut fermer
                        auto_checkout = (pointage.timestamp - timedelta(minutes=1)).strftime('%Y-%m-%d %H:%M:%S')
                    else:
                        # Le nouveau pointage est AVANT le check-in ouvert
                        # Ferme avec check_in + 8h (journée de travail standard)
                        auto_checkout = (open_checkin + timedelta(hours=8)).strftime('%Y-%m-%d %H:%M:%S')

                    self.odoo_client.update_attendance_checkout(
                        attendance_id=open_attendance['id'],
                        check_out=auto_checkout,
                    )
                    print(f"    Auto-fermeture présence {open_attendance['id']} pour {pointage.employee_name}")

                # Crée le nouveau check-in
                ...
                # Check-out: cherche une présence ouverte à fermer
                open_attendance = self.odoo_client.get_open_attendance(odoo_emp_id)
                ...
                if open_attendance:
                    ...
                    # Pas de présence ouverte, ignore le check-out orphelin
                    ...
                    return SyncResult(
                        ...
                        employee_name=pointage.employee_name,
                        action='skipped',
                        error='Pas de check-in correspondant',
                    )
    ...
    def export(self, results: List[SyncResult], **kwargs) -> Path:
        ...
        # Append au log existant
        log_data = []
        if self.sync_log_file.exists():
            try:
                with open(self.sync_log_file, 'r') as f:
                    log_data = json.load(f)
                    # Garde les 100 derniers logs
    ...
    def print_summary(self, data: Any, results: Any):
        ...
        print(f"  ⚠️ Non matchés: {self.stats.skipped_no_match}")
...
def run_daemon(interval_minutes: int = None):
    ...
    Args:
        interval_minutes: Intervalle entre les syncs (défaut: config)
    """
    ...
    # Lance immédiatement puis à intervalle régulier
